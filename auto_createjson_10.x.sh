...
path=/home/androed/crd14                                                            # Here you will need to specify the path to the crDroid source code folder
...
recovery=$(basename $path/out/target/product/$device/recovery.img)                          # Here we get the recovery file
...
gapps="https://github.com/BiTGApps/BiTGApps-Release/releases/download/v2.7/BiTGApps-arm-14.0.0-v2.7-CORE.zip" #https link (leave empty if unused)
...
gh release create $nozip --notes "Automated release crDroid for $device $version $date/$time" $path/out/target/product/$device/$zip $path/out/target/product/$device/$recovery
